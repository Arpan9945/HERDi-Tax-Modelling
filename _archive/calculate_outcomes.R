calculate_outcomes <- function(health_data, diseases, market_base, market_new, tax_inc_rate, 
                               gdp_per_capita = 1447.3, verbose = TRUE) {
  ...
  # 3. Aggregate the totals back into a clean breakdown table
  ...
  # 4. Calculate total overall impact
  ...
  # Attach totals to health_data so your Monte Carlo doesn't break
  health_data$Total_DALYs_Saved <- total_dalys_saved
  health_data$Econ_Value_USD <- total_econ_value
  ...
  return(list(Detailed_Health = health_data, Disease_Breakdown = disease_breakdown, 
            Revenue_Gain = revenue_gain, Baseline_Revenue = old_revenue, New_Revenue = new_revenue))
}
