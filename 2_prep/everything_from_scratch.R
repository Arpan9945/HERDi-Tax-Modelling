...
df_population  <- subset(df_population,  `Age-Group` != "15-19")
df_consumption <- subset(df_consumption, `Age-Group` != "15-19")
  
#Step 1:


elasticity_matrix <- matrix(
  c(-1.20,  0.15,  0.10,  0.08,   # Carbonated: own=-1.20 (Bahl et al.), cross = weak substitutes
     0.10, -0.75,  0.08,  0.05,   # Juice: own=-0.45 (Bahl et al.), cross = weak substitutes
     0.08,  0.10, -1.12,  0.06,   # Milk-Sweetened: own=-1.12 (Bahl et al.), cross = weak substitutes
     0.10,  0.08,  0.06, -0.90),  # Energy drinks: own=-0.90 (Huque et al.), cross = weak substitutes
...
GDP_Per_Capita_USD <- 213476.75 #using this value now
...
for(i in 1:length(tax_rates)){
  for(j in 1:length(pass_throughs)){
    ...
    mat_dalys[i,j] <- sum(temp_outcomes$Total_DALYs_Saved)
...
for(i in 1:n_runs){
  ...
  sim_dalys[i] <- sum(sim_outcomes$Total_DALYs_Saved)
