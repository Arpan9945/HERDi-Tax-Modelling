...
# 3. Calculate the model variables!
disease_catalog_age <- clean_gbd %>%
  mutate(
    ...
    DALYs_per_Case = `DALYs (Disability-Adjusted Life Years)_Number` / Incidence_Number
  ) %>%
  
  # 4. Clean up names to match the existing model
  rename(
    Disease_Name = cause_name,
    Sex = sex_name
  ) %>%
  
  # 5. Format the Age-Group string to match the df_population (e.g., "20 to 24" -> "20 - 24")
  mutate(
    `Age-Group` = str_replace(age_name, " years", "")
  )
